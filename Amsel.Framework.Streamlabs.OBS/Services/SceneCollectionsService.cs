...
namespace Amsel.Framework.Streamlabs.OBS.Services
{
    public class SceneCollectionsService
    {
        ...
        {
            return client.SendRequest<StreamlabsOBSEvent>(new StreamlabsOBSRequest("fetchSceneCollectionsSchema", RESOURCE))?.FirstOrDefault()
        ...
        public SceneCollectionSchema FetchSchemaForCollectionById([NotNull] string id)
        {
            ...
            return FetchSchema()?.Where(x => x.Id == id).PickRandom<SceneCollectionSchema>();
        }

        public SceneCollectionSchema FetchSchemaForCollectionByName(string name)
        {
            if (name == null) throw new ArgumentNullException(nameof(name));
            return FetchSchema()?.Where(x => x.Name == name).PickRandom<SceneCollectionSchema>();
        }
